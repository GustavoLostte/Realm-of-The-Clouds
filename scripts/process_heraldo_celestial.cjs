...
async function processHeraldo() {
  const src = '/Users/wizzard/.gemini/antigravity-ide/brain/e08b0e2d-ff7b-4eae-910b-8d5305d04b22/heraldo_concept_02_1789767998186.jpg';
  if (!fs.existsSync(src)) {
    throw new Error('Source file not found: ' + src);
  }

  // 1. Guardar el archivo chroma original
  const chromaDest1 = path.resolve(__dirname, '../public/assets/npcs/chroma/heraldo_celestial.jpg');
  const chromaDest2 = path.resolve(__dirname, '../public/assets/characters/chroma/heraldo_celestial.jpg');
  fs.mkdirSync(path.dirname(chromaDest1), { recursive: true });
  fs.mkdirSync(path.dirname(chromaDest2), { recursive: true });
  ...
  console.log('Dimensiones del Heraldo:', { cropW, cropH, minX, minY });

  // Escalar para lienzo de 1024x1024
  const targetH = 920;
  ...
  const offY = 1024 - targetH - 40; // Apoyado hacia abajo
  ...
  const targets = [
    'public/assets/npcs/heraldo_celestial.webp',
    'public/assets/characters/fullbody_cutout/heraldo_celestial.webp',
    'public/assets/structures/cutout/heraldo_celestial.webp',
    'public/assets/characters/fullbody_cutout/heraldo_alfombra_roja.webp',
    'public/assets/npcs/heraldo_alfombra_roja.webp'
  ];
  ...
  const artifactPath = '/Users/wizzard/.gemini/antigravity-ide/brain/e08b0e2d-ff7b-4eae-910b-8d5305d04b22/heraldo_celestial_cutout.webp';
  fs.writeFileSync(artifactPath, master1024);
  console.log('✓ Guardado en artifacts:', artifactPath);
}

processHeraldo().catch(console.error);
