...
CONFIGS = {
    'male': {
        ...
        'primary_id': 'knight_male',
        'aliases': ['kina_male', 'Kina_male'],
        'cx': 949,
        'top_y': 20,
        'crop_size': 380,
        'duration_ms': 40,
    ...
    'female': {
        'src_dir': os.path.join(STARTER_DIR, 'kina_female/idle'),
        'primary_id': 'knight_female',
        'aliases': ['kina_female', 'KINA_FEMALE'],
        'cx': 870,
        'top_y': 140,
        'crop_size': 360,
        'duration_ms': 40,
...
def process_knight(gender, cfg):
    ...
    print(f"\n==========================================")
    print(f"Processing Knight {gender} ({champ_id}) - {len(files)} frames...")
    print(f"==========================================")
    ...
    # Master directory in starter/<primary_id>/
    ...
    # Also save kina_male / kina_female at root
    kina_id = f"kina_{gender}"
    with open(os.path.join(CHAMPIONS_DIR, f"{kina_id}_idle.webp"), 'wb') as f:
        f.write(anim_data)
    with open(os.path.join(CHAMPIONS_DIR, f"{kina_id}_poster.webp"), 'wb') as f:
        f.write(poster_data)
    with open(os.path.join(CHAMPIONS_DIR, f"{kina_id}_avatar.webp"), 'wb') as f:
        f.write(avatar_data)
...
if __name__ == '__main__':
    for gender, cfg in CONFIGS.items():
        process_knight(gender, cfg)
    print("\nBoth knights (Kina male & female) successfully processed and integrated!")
